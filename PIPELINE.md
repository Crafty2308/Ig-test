...
The Crush Depth tick inside a well lands as `ground`.
...
|---|---|---|
| Multishot (`projectiles`) | minion, blade, dash, blast, chain, burn, frost, aura, ground, thorns, tether, onkill | These have no emission step to duplicate. Thrall strikes are melee, blades scale on `orbitCount`, explosions on `clusterCount`, thralls on `minionCount` and burning ground on `fireSize`. **Applies to `beam`** — multishot splits it into extra rays. |
| Pierce | minion, blade, dash, blast, chain, burn, frost, aura, ground, thorns, tether, onkill | No travel path to pass through. **Applies to `beam`** — pierce is how many bodies one ray damages. |
...
riders across all live sources, and that drone shots carry multishot, pierce,
